package me.specifies.core.Constants;

public interface InventoryMessages {
	
	// Preferences
	public static final String PREFERENCES_TITLE = "&cPreferences";
	public static final String STATUS_ON = "&7Discord Status&8: &aOn.";
	public static final String STATUS_OFF = "&7Discord Status&8: &cOff.";
	public static final String MESSAGE_ON = "&7Linked Discord Chat&8: &aOn.";
	public static final String MESSAGE_OFF = "&7Linked Discord Chat&8: &cOff.";
	public static final String CHAT_ON = "&7Chat Verification Message&8: &aOn.";
	public static final String CHAT_OFF = "&7Chat Verification Message&8: &cOff.";

}
